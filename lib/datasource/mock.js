...
export async function cases(diseaseName) {
  ...
  const localCases = confirmed ? rand(0, confirmed) : 0;
  const importedCases = Math.max(0, confirmed - localCases);
  ...
  return {
    ...
    newAdmissions24h: admitted ? rand(0, Math.min(2, admitted)) : 0,
    newRecoveries24h: recoveries ? rand(0, Math.min(2, recoveries)) : 0,
    newDeaths24h: deaths ? rand(0, Math.min(1, deaths)) : 0,
    contactsListed,
    contactsFollowedUp,
    importedCases,
    localCases,
